...
public abstract class RegionAggregation extends H2Operator {
    ...
    protected static Map<String,Map<String,TreeSet<DatabaseTimePeriod>>> aggregationsKeysUEMOA;
    protected static Map<String,Map<String,TreeSet<DatabaseTimePeriod>>> aggregationsKeysEAC;
    protected static Map<String,Map<String,TreeSet<DatabaseTimePeriod>>> aggregationsKeysSADC;
    protected static Map<String,Map<String,TreeSet<DatabaseTimePeriod>>> aggregationsKeysECO;
    ...
    protected void initAggregationsKeys(Region region) throws SQLException {
        OGraphDatabase database = OrientServer.getDatabase(
                initProperties.getProperty("CountrySTAT.aggregations.keyProductsDatabase.url"),
                initProperties.getProperty("CountrySTAT.aggregations.keyProductsDatabase.usr"),
                initProperties.getProperty("CountrySTAT.aggregations.keyProductsDatabase.psw")
        );
        Map<String,Map<String,TreeSet<DatabaseTimePeriod>>> aggregationsKeys = null;
        switch (region) {
        ...
        try {
            ...
            if (aggregationsKeys==null || (currentLastUpdate!=null && (lastUpdate==null || currentLastUpdate.compareTo(lastUpdate)>0))) {
                ...
                aggregationsKeys = new HashMap<String, Map<String, TreeSet<DatabaseTimePeriod>>>();
                ...
                for (ODocument rowO : aggregationsKeyO) {
                    ...
                    Map<String,TreeSet<DatabaseTimePeriod>> aggregationsItemsKeys = aggregationsKeys.get(geo);
                    if (aggregationsItemsKeys==null)
                        aggregationsKeys.put(geo, aggregationsItemsKeys = new HashMap<String, TreeSet<DatabaseTimePeriod>>());
                    TreeSet<DatabaseTimePeriod> aggregationsPeriodsKeys = aggregationsItemsKeys.get(item);
                    if (aggregationsPeriodsKeys==null)
                        aggregationsItemsKeys.put(item, aggregationsPeriodsKeys = new TreeSet<DatabaseTimePeriod>());
                    aggregationsPeriodsKeys.add(new DatabaseTimePeriod((Long)rowO.field("TIME_FROM"),(Long)rowO.field("TIME_TO")));
                }
                switch (region) {
    ...
    protected static boolean isKeyProduct(Region region, String geo, String item, Long year) {
        boolean key = false;
        Map<String,TreeSet<DatabaseTimePeriod>> aggregationsItemsKeys = null;
        switch (region) {
        ...
        if (aggregationsItemsKeys!=null) {
            TreeSet<DatabaseTimePeriod> aggregationsPeriodsKeys = aggregationsItemsKeys.get(item);
            if (aggregationsPeriodsKeys!=null)
                return aggregationsPeriodsKeys.contains(new DatabaseTimePeriod(year, year));
        }
        return false;
    }
